	...
	class record{
		...
		function displayReports($page,$appObj){
			if(empty($this->pastReports)){
				...
				echo "<p class=\"error\">No reports available</p>";
			}else{
                                echo "<div class=\"formContainer\"><table><tr><th>Date</th>";
               
                                echo "<th></th></tr>";
				foreach ($this->pastReports as $report) {
					$reportObject = base64_encode(serialize($report));
                                        
                                        echo "<tr><td>".$report->viewDate()."</td>";
                                        echo "<td><button type=\"button\" class=\"editbtn\" name=\"view\" onClick=\"location.href='{$page}?date={$report->viewDate()}&&report={$reportObject}&&appObj={$appObj}'\">View Record</button></td></tr>";
