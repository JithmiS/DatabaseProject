...
if (isset($_POST['login'])) {
    ...
    if (empty($errors)) {
        ...
        $query = "SELECT * FROM customers WHERE email='{$email}' AND password=password({$password}) AND is_deleted=0 LIMIT 1";
        ...
        $query = "SELECT * FROM drivers WHERE email='{$email}' AND password=password({$password}) AND is_deleted=0 LIMIT 1";
        $result_set = mysqli_query($connection, $query);
        verifyQuery($result_set);
        if (mysqli_num_rows($result_set) == 1) {
        ...
        $query = "SELECT * FROM assistantDrivers WHERE email='{$email}' AND password=password({$password}) AND is_deleted=0 LIMIT 1";
        $result_set = mysqli_query($connection, $query);
        verifyQuery($result_set);
        if (mysqli_num_rows($result_set) == 1) {
