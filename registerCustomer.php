...
if (isset($_POST['register'])) {
    ...
    if (mysqli_num_rows($resultSet) > 0) {
      ...
      if (empty($errors)) {
        ...
        $type = $_POST['type'];
		$query = "INSERT INTO users(email,first_name,last_name,address,contact_number,password,is_deleted) VALUES('{$email}','{$firstname}','{$lastname}','{$address}','{$contact_number}',password({$password}),0)";
        $result = mysqli_query($connection, $query);
        verifyQuery($result);
		$query = "INSERT INTO customers(email,customer_role) VALUES('{$email}','{$type}')";
        $result = mysqli_query($connection, $query);
        verifyQuery($result);
